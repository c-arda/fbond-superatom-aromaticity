...
# -*- coding: utf-8 -*-
"""
Basis-set comparison: Al4(2-) CCSD/def2-TZVP vs def2-SVP
==========================================================
Runs Al4(2-) at def2-TZVP to provide a second regime validation point.
This addresses the reviewer concern about whether the f_e regime gap
(small cluster ~0.08 vs superatom ~0.04) is a basis-set artifact.

Expected runtime: ~10-30 min (4 atoms, 54 electrons).
"""
...
# ============================================================================
# Al4(2-) GEOMETRY — same as SVP calculation (square planar D4h)
# ============================================================================

AL4_GEOMETRY = """
Al   0.0000   0.0000   0.0000
Al   2.5000   0.0000   0.0000
Al   2.5000   2.5000   0.0000
Al   0.0000   2.5000   0.0000
"""


def run_al4_ccsd(basis='def2-tzvp'):
    """Run CCSD N_D analysis for Al4(2-) at given basis set."""

    t_start = time.time()

    print("=" * 70)
    print(f"  N_D Basis-Set Comparison: Al4(2-)")
    ...
    n_ao = mol.nao
    # Freeze Al 1s core orbitals (4 aluminum atoms)
    n_frozen = 4
    ...
    print(f"\n  {'═' * 50}")
    print(f"  RESULTS for Al4(2-) CCSD/{basis}:")
    print(f"  {'═' * 50}")
    print(f"  N_e             = {n_elec}")
    ...
    outfile = f'data/Al4_2minus_{basis.replace("-","")}_results.json'
    with open(outfile, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\n  Saved: {outfile}")

    # ----- SVP comparison -----
    svp_nd = 3.84018985065777
    svp_fe = svp_nd / 46  # N_corr = 54 - 2*4 = 46
